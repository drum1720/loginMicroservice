...
func ParseData(body io.ReadCloser, essence core.Validator) *response.Error {
	...
	if err = json.Unmarshal(buffer, &essence); err != nil {
		...
		return response.NewError(errors.New("unmarshal err"), http.StatusInternalServerError)
	}
